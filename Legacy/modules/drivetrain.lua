...
M.params = {
    ...
    usePowerLut = true,
    usePrototypeEngine = true,
    prototypeAuthorityFallback = 0.35,
    prototypeTorqueKey = "ngp_engine_proto_output_torque_nm",
    prototypeAuthorityKey = "ngp_engine_proto_authority",
    usePrototypeClutch = true,
    prototypeClutchTorqueKey = "ngp_clutch_p2_transmitted_torque_nm",
    usePrototypeGearbox = true,
    prototypeGearboxOutputTorqueKey = "ngp_gearbox_p3_output_torque_nm",
...
local state = {
    ...
    legacyEngineTorqueNm = 0.0,
    prototypeTorqueNm = 0.0,
    prototypeAuthority = 0.0,
    clutchTorqueNm = 0.0,
...
local function updateTorqueNmPath(dt)
    ...
    local legacyEngineNm = calculateEngineTorqueNm(state.gas, state.filteredRPM, state.gear)
    state.legacyEngineTorqueNm = legacyEngineNm
    local engineNm = legacyEngineNm

    if M.params.usePrototypeEngine then
        local prototypeNm = safeLoadNumber(M.params.prototypeTorqueKey, legacyEngineNm)
        local authority = clamp(safeLoadNumber(M.params.prototypeAuthorityKey, M.params.prototypeAuthorityFallback), 0.0, 1.0)
        engineNm = legacyEngineNm * (1.0 - authority) + prototypeNm * authority
        state.prototypeTorqueNm = prototypeNm
        state.prototypeAuthority = authority
    else
        state.prototypeTorqueNm = legacyEngineNm
        state.prototypeAuthority = 0.0
    end

    local clutchFactor = clamp(state.clutch or 0.0, 0.0, 1.0)
    local clutchNm
    if M.params.usePrototypeClutch then
        local p2 = safeLoadNumber(M.params.prototypeClutchTorqueKey, nil)
        if p2 ~= nil then
            -- P02 reports the torque transmitted through the clutch. Its
            -- sign follows the engine-to-gearbox torque direction.
            clutchNm = clamp(p2, -M.params.clutchTorqueCapacityNm, M.params.clutchTorqueCapacityNm)
        end
    end
    if clutchNm == nil then
        ...
        clutchNm = engineNm * clutchFactor * (1.0 - clutchSlipLoss)
    end

    local gearboxNm = clutchNm * gearRatio
    if M.params.usePrototypeGearbox then
        local p3 = safeLoadNumber(M.params.prototypeGearboxOutputTorqueKey, nil)
        local p3Count = safeLoadNumber("ngp_gearbox_p3_update_count", 0.0)
        if p3 ~= nil and p3Count > 1.0 then
            gearboxNm = clamp(p3, -M.params.maxDiffInputTorqueNm, M.params.maxDiffInputTorqueNm)
        end
    end
...
local function exportState()
    ...
    safeStore("ngp_legacy_engine_torque_nm", state.legacyEngineTorqueNm or 0.0)
    safeStore("ngp_prototype_engine_torque_nm", state.prototypeTorqueNm or 0.0)
    safeStore("ngp_engine_torque_authority", state.prototypeAuthority or 0.0)
    safeStore("ngp_drivetrain_p2_clutch_enabled", M.params.usePrototypeClutch and 1 or 0)
